...
alter table [dbo].[stops_tpa_staging]
add RecID int IDENTITY(1,1) NOT NULL,
CONSTRAINT [PK_stops_tpa_staging] PRIMARY KEY CLUSTERED 
(
	RecID ASC
...
create spatial index spin_Stops ON [dbo].[stops_tpa_staging](Shape);
--Create index on compare columns (agency_stop_id)
create index IX_StopID on [dbo].[stops_tpa_staging](agency_stop_id)
...
With OriginStops AS
	...
	* from [dbo].[stops_tpa_staging])
	...
	CROSS APPLY (Select Top(1) * from [dbo].[stops_tpa_staging] as DD
...
select agency_stop_id,
	...
	--stops_tpa_staging ON stops_tpa_staging.agency_stop_id = DS.agency_stop_id
	UPDATE       stops_tpa_staging
	SET                Distance_Eligible = 1
	FROM            #StopsThatMeetDistanceThreshold INNER JOIN
	                       stops_tpa_staging ON stops_tpa_staging.agency_stop_id = #StopsThatMeetDistanceThreshold.agency_stop_id
